...
const char * const BITCOIN_CONF_FILENAME = "superdigitalcoin.conf";
const char * const BITCOIN_PID_FILENAME = "superdigitalcoin.pid";
...
#else
    const char* pszModule = "superdigitalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuperDigitalCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SuperDigitalCoin";
#else
    // Unix
    return pathRet / ".superdigitalcoin";
#endif
#endif
}
