...
*Setup instructions for a Gitian build of Super Digital Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SuperDigitalCoin
...
- [Building Super Digital Coin Core](#building-superdigitalcoin-core)
...
Clone the git repositories for superdigitalcoin and Gitian.
...
in the superdigitalcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Super Digital Coin Core
----------------

To build Super Digital Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the superdigitalcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/superdigitalcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/superdigitalcoin-project/superdigitalcoin
...
For example:
```bash
URL=https://github.com/thrasher-/superdigitalcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit superdigitalcoin=${COMMIT} --url superdigitalcoin=${URL} ../superdigitalcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit superdigitalcoin=${COMMIT} --url superdigitalcoin=${URL} ../superdigitalcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit superdigitalcoin=${COMMIT} --url superdigitalcoin=${URL} ../superdigitalcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the superdigitalcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../superdigitalcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/superdigitalcoin
SIGPATH=/some/root/path/superdigitalcoin-detached-sigs

./bin/gbuild --url superdigitalcoin=${BTCPATH},signature=${SIGPATH} ../superdigitalcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/superdigitalcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/superdigitalcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/superdigitalcoin-osx-build.assert
```
...
[superdigitalcoin-project/gitian.sigs.SDC](https://github.com/superdigitalcoin-project/gitian.sigs.SDC/) repository, or if that's not possible create a pull
